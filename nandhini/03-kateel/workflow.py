...
# 1. OpenAI (Uses LangChain's default retry mechanism, increased retries)
...
# 2. Anthropic (Uses LangChain's default retry mechanism, increased retries)
anthropic_llm = ChatAnthropic(model="claude-3-5-sonnet-20240620", max_retries=5)

# 3. Gemini (Uses Google API's internal retry mechanism, increased retries)
...
async def debate_director_node(state: CollaborativeState, config: RunnableConfig) -> dict:
    ...
    messages = get_step_messages(state, lead_agent_name, DEFAULT_LEAD_AGENT_CONTRIBUTIONS_LAST, 1) # Only need the most recent contributions for decision
    ...
    if "CONTINUE_DEBATE" in decision:
        ...
        parsed_decision = "PRESENT_FINDINGS" # Default to presenting results to avoid infinite loops
    ...
    update_step = {
        ...
        "category": "lead",
        "decision": parsed_decision # Custom field to store the routing decision
    }
...
def debate_director_router(state: CollaborativeState) -> DebateAction:
    ...
    """
    # The last message in the state is the output from the debate_director_node
    # We read the content of this message (which is the decision keyword)
    last_message = state["messages"][-1].content
    
    if last_message == "CONTINUE_DEBATE":
        return "CONTINUE_DEBATE"
    
    # Default is PRESENT_FINDINGS
...
def create_graph() -> Tuple[StateGraph, CompiledStateGraph]:
    ...
    # 2. Conditional edge from the director node, using the new router function
    workflow.add_conditional_edges(
        "debate_director_node",
        debate_director_router, # Use the new router function to read the state
        {
