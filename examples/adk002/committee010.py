...
import requests # Added for Tavily Search
...
# Ensure LiteLlm is available for non-native ADK models like those accessed via Ollama/Groq
from google.adk.models.lite_llm import LiteLlm 

# --- Enhanced Documentation (Updated for Grok) ---
"""
...
consensus reports. Supports cloud models (Claude, Gemini, Grok) and local models 
(DeepSeek via Ollama).

SETUP INSTRUCTIONS:
------------------
1. Install required packages:
    pip install google-adk python-dotenv litellm requests
    # litellm is required for seamless integration of non-native models like Ollama/Groq

2. Set up Ollama and DeepSeek (if using deepseek-local):
    a. Install and run Ollama: https://ollama.com/
    b. Pull the DeepSeek model: ollama pull deepseek-coder:6.7b

3. Set up API keys in .env file or environment:
    ANTHROPIC_API_KEY=your-anthropic-key-here    # Required for Claude
    GOOGLE_API_KEY=your-google-key-here          # Required for Gemini
    GROQ_API_KEY=your-groq-key-here              # **NEW: Required for Grok**
    TAVILY_API_KEY=your-tavily-key-here          # Required for Claude/Grok/Ollama web search
...
Specify Grok AI provider:
  python committee_debate.py "topic1" "topic2" --usegenai grok
...
-----------------------
  ...
  gemini          Gemini 3 Pro Preview (requires GOOGLE_API_KEY)
  gemini-flash    Gemini 2.5 Flash Experimental (requires GOOGLE_API_KEY)
  grok            **NEW: Grok-1.5 Standard (requires GROQ_API_KEY)**
  grok-1-5        **NEW: Grok-1.5 Standard (requires GROQ_API_KEY)**
...
- Web search capability for agents (Google for Gemini, Tavily for others)
- Support for multiple AI providers (cloud and local via Ollama)

CHANGELOG:
----------
v2.2 - **NEW: Added Grok AI support via LiteLLM.**
...
# --- Tavily Search Integration (Requires 'requests' library) ---
def tavily_search(query: str) -> str:
    ...
    try:
        # Note: Must import 'requests' at the top of the file
...
# --- Hybrid Debate Agent (Modified to include 'groq/') ---
def create_debate_agent(index: int, topic_str: str, model) -> LlmAgent:
    tools_list = []
    search_context = ""
    # Model can be a string (for native Gemini) or a LiteLlm object (for others)
    model_name = str(model.model) if isinstance(model, LiteLlm) else str(model)

    # Hybrid logic: Gemini supports native Google Search tool, others use Tavily API
    if model_name.startswith("gemini"):
        tools_list = [google_search]  # Gemini supports Google Search
    # **MODIFIED: Added 'groq/' to the list for LiteLlm/Tavily search**
    elif any(model_name.startswith(p) for p in ["anthropic/", "groq/", "ollama/"]):
        search_context = tavily_search(topic_str)  # Claude/Grok/Ollama use Tavily
...
def create_consensus_agent(agent_count: int, prior_reports: str, new_inputs: str, model) -> LlmAgent:
...
def build_debate_coordinator(agent_count: int, topic_str: str, prior_reports: str, model) -> SequentialAgent:
    # Ensure the model is passed consistently to the debate and consensus agents
...
async def main(topics: list[str], agent_count: int, topic_name: str, use_genai: str):
    ...
    model_map = {
        ...
        # 🌟 NEW: Grok via LiteLLM (Groq provider, requires GROQ_API_KEY)
        "grok": "groq/grok-1.5-standard",
        "grok-1-5": "groq/grok-1.5-standard",
        # DeepSeek via LiteLLM (Ollama provider)
    ...
    if provider_lower not in model_map:
        ...
        print(f"   - **Grok models require: GROQ_API_KEY and TAVILY_API_KEY**") # **UPDATED**
        print(f"   - deepseek-local requires: Ollama running with 'deepseek-coder:6.7b' pulled, and TAVILY_API_KEY.")
        sys.exit(1)
    
    model_str = model_map[provider_lower]
    
    # Wrap LiteLLM-based models (including Claude, Grok, and Ollama)
    if any(p in model_str for p in ["anthropic/", "groq/", "ollama/"]): # **MODIFIED: Added 'groq/'**
        
        if model_str.startswith("ollama/"):
            print(f"💡 Running {model_str.split('/')[-1]} locally. Ensure Ollama is running.")
        
        # **NEW: Check for GROQ_API_KEY if Grok is selected**
        ...
        print(f"🤖 Using {use_genai.upper()} AI via LiteLLM")
        print(f"   Model: {model_str}")
    else:
        # For native ADK models (Gemini)
        model = model_str
        print(f"🤖 Using {use_genai.title()} AI")
        print(f"   Model: {model}")
    ...
    # Pass the potentially wrapped model object to the coordinator
...
if __name__ == "__main__":
    ...
    if "--usegenai" in args:
        ...
        try:
            ...
            print("Error: '--usegenai' must be followed by a provider name (e.g., 'claude' or 'grok').") # **UPDATED**
            sys.exit(1)
    ...
    if not topics:
        ...
        print("\nAvailable Providers (v2.2):") # **UPDATED**
        ...
        print("   gemini-flash        Gemini 2.5 Flash Experimental")
        print("   grok                **NEW: Grok-1.5 Standard (requires GROQ_API_KEY)**") # **NEW**
        print("   grok-1-5            **NEW: Grok-1.5 Standard (requires GROQ_API_KEY)**") # **NEW**
        print("   deepseek-local      DeepSeek Coder 6.7B via local Ollama instance")
        print("\nEnvironment Variables Required:")
        print("   ANTHROPIC_API_KEY   For Claude models")
        print("   GOOGLE_API_KEY      For Gemini models")
        print("   **GROQ_API_KEY** **For Grok models**") # **NEW**
        print("   **TAVILY_API_KEY** **For Claude, Grok, and DeepSeek web search**") # **UPDATED**
        sys.exit(1)
