...
def load_previous_reports() -> str:
    reports = sorted(glob.glob("committeereport*.txt"))
    combined = ""
...
def save_committee_report(content: str) -> str:
    existing = sorted(glob.glob("committeereport*.txt"))
    next_index = len(existing) + 1
    filename = f"committeereport{next_index:03d}.txt"
    try:
...
async def main(topics: list[str], agent_count: int = 3):
    ...
    prior_reports = load_previous_reports()
    ...
    save_committee_report(full_report)
    print("\n🧾 Final Consensus:\n" + (consensus_text or "No consensus generated."))

# --- CLI Entry Point ---
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python committee_debate.py <topic1> <topic2> ... [--agents N]")
        sys.exit(1)
